...
benchmarkType = ["table", "shelf"]
scenarios = ["1", "2"]
nroadmapPerParam = 15 ## can increase later if have more time
...
for bt in benchmarkType:
	...
	################################
	if bt == "table":
		benchmarkType_path = "benchmark/table"
		try:
		...
		benchmarkType_path = "benchmark/shelf"
		try:
			os.mkdir(benchmarkType_path)
		except OSError:
			print "Creation of the directory %s falied\n" % benchmarkType_path
	...
	for scene in scenarios:
		...
		truePoses, nObjectInExecuting = utils.trueScene_generation(bt, scene, Objects, executingServer)
		...
		for nh in nHypos:
			...
			while nn < nroadmapPerParam+1:
				...
				meshSet, nObjectInPlanning = utils.planScene_generation(Objects, bt, static_geometries_planning, 
												hypo_roadmaps_folder, nh, noiseLevel_stardard, cc, nn, planningServer)
				nsamples = int(sys.argv[1])
				utils.roadmap_generation(home_configuration, kukaID_p, kuka_ee_idx, ll, ul, jr,
				...
				utils.deletePlanScene(meshSet, planningServer)
				print "start planning..."
				executeFile = "../robust_planning_20_icra/main" + " " + str(bt) + " " + str(scene) + " " \
				...
				if failure_indicator[0] == "0":
					...
					utils.executeAllTraj(home_configuration, kukaID_e, hypo_traj_folder, cc, nn, executingServer)
					nn += 1
				else:
		...
		for nol in noiseLevel:
			...
			while nn < nroadmapPerParam+1:
				...
				if failure_indicator[0] == "0":
					...
					## Now we should have the corresponding trajectory in the "trajectories" folder
					## Execute these trajectories (5 trajs)
					utils.executeAllTraj(home_configuration, kukaID_e, noiseLevel_traj_folder, cc, nn, executingServer)
					nn += 1
				else:
					print "At least one of the search fails. We will not execute this. Generate another scene."
		...
		utils.deleteTruePoses(truePoses, executingServer)

	## finish the current benchmark type, delete static_geometries for both true and planning scene
	utils.deleteStaticGeometries(static_geometries_planning, planningServer)
	utils.deleteStaticGeometries(static_geometries_executing, executingServer)
